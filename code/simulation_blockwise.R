#simulation with block-wise pattern#
library("DirichletReg")
library("NMF")
library("combinat")
pattern<-matrix(ncol=50,nrow=5)
pattern[1,]<-c(0.02736860,0.12541887,0.05641219,0.07464338,0.09908623,0.06283853,0.16954483,0.08033740,0.12642745,0.17792252,rep(0,40))
pattern[2,]<-c(rep(0,10),0.05518038,0.03670391,0.20534650,0.01779860,0.19465400,0.15284080,0.12575085,0.02593506,0.11578989,0.07000000,rep(0,30))
pattern[3,]<-c(rep(0,20),1.713488e-01,5.411259e-02,1.476365e-03,2.709419e-03,1.494267e-01,6.800434e-02,1.616755e-13,3.024545e-01,1.602027e-13,2.504672e-01,rep(0,20))
pattern[4,]<-c(rep(0,30),0.03419887,0.02268002,0.05456208,0.03440245,0.19438291,0.21710751,0.02188385,0.13304830,0.16673734,0.12099668,rep(0,10))
pattern[5,]<-c(rep(0,40),0.02736860,0.12541887,0.05641219,0.07464338,0.09908623,0.06283853,0.16954483,0.08033740,0.12642745,0.17792252)
...
pattern_rank<-matrix(ncol=10,nrow=5)
for (i in 1:5){
	pattern_rank[i,1:10]<-c(1:10)+(i-1)*10
}
for (time in 1:100){
	poisson_num<-10
    ...
    for (i in 1:5){
    	w1_final_rank<-order(w1_final[,i],decreasing=T)[1:10]
